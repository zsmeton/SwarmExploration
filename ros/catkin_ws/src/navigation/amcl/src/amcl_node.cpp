...
#include "amcl_msgs/WeightedParticlecloud.h"
#include "amcl/SetParticlecloud.h"
#include "amcl/GetParticlecloud.h"
...
class AmclNode
{
  ...
  private:
    ...
    bool getParticlecloudCallback(amcl::GetParticlecloud::Request& req,
                                  amcl::GetParticlecloud::Response& res);
    bool setParticlecloudCallback(amcl::SetParticlecloud::Request& req,
                                  amcl::SetParticlecloud::Response& res);
    ...
    ros::ServiceServer set_particlecloud_srv_;
    ros::ServiceServer get_particlecloud_srv_;
        ...
        first_reconfigure_call_(true)
{
  ...
  get_particlecloud_srv_ = nh_.advertiseService("get_particlecloud", &AmclNode::getParticlecloudCallback, this);
  set_particlecloud_srv_ = nh_.advertiseService("set_particlecloud", &AmclNode::setParticlecloudCallback, this);
...
bool
AmclNode::getParticlecloudCallback(amcl::GetParticlecloud::Request& req,
                         amcl::GetParticlecloud::Response& res)
{ 
  pf_sample_set_t* set = pf_->sets + pf_->current_set;

  res.cloud.header.stamp = ros::Time::now();
  res.cloud.header.frame_id = global_frame_id_;
  
  res.cloud.poses.resize(set->sample_count);
  res.cloud.weights.resize(set->sample_count);
  for(int i=0;i<set->sample_count;i++)
  {
    tf::poseTFToMsg(tf::Pose(tf::createQuaternionFromYaw(set->samples[i].pose.v[2]),
                    ...
                    res.cloud.poses[i]);
    res.cloud.weights[i] = set->samples[i].weight;
  }
...
bool
AmclNode::setParticlecloudCallback(amcl::SetParticlecloud::Request& req,
                         amcl::SetParticlecloud::Response& res)
{ 
  // TODO: Add check on the timestamp for the data

  bool resampled = false;
  std::vector<geometry_msgs::Pose> cloud = req.cloud.poses;
  std::vector<double> weights = req.cloud.weights;
  pf_sample_set_t* set = pf_->sets + pf_->current_set;
  pf_sample_t *sample;
  ...
  for(int i=0;i<set->sample_count;i++)
  {
    ...
    // Set sample positions and yaw and weight
    set->samples[i].pose.v[2] = yaw;
    set->samples[i].pose.v[0] = position.getX();
    set->samples[i].pose.v[1] = position.getY();
    set->samples[i].weight = weights.at(i);
  }
  ...
  if(resampled)
  {
    ...
    if(max_weight > 0.0)
    {
      ...
      p.header.stamp = req.cloud.header.stamp;
      // Copy in the pose
      ...
      if (tf_broadcast_ == true)
      {
        ...
        ros::Time transform_expiration = (req.cloud.header.stamp +
