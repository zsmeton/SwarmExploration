...
if __name__ == "__main__":
    ...
    error_sum_local = 0
    num_time_local = 0
    for test in range(start_test, num_tests+1, 2):
        for robot, marker in zip(['tb3_1', 'tb3_2', 'tb3_3'], ['1', '2', '3']):
            ...
            #print([(a,b) for a,b in zip(amcl_pose[:,2], actual_pose[:,2])])
    
    error_sum_dist = 0
    num_time_dist = 0
    for test in range(start_test+1, num_tests+1, 2):
        for robot, marker in zip(['tb3_1', 'tb3_2', 'tb3_3'], ['1', '2', '3']):
            time, amcl_pose, actual_pose = get_data(test, robot)
            num_time_dist += len(time)
            error_sum_dist += np.sum(np.linalg.norm(amcl_pose[:, 0:2] - actual_pose[:, 0:2], axis=-1))
    
    print(f'Local Error: {error_sum_local/(num_time_local)}')
    print(f'Distributed Error: {error_sum_dist/(num_time_dist)}')
    
    for test in range(45, num_tests + 1):
        fig, ax = plt.subplots()
        for robot, marker in zip(['tb3_1', 'tb3_2', 'tb3_3'], ['1', '2', '3']):
            time, amcl_pose, actual_pose = get_data(test, robot)
            ax.scatter(time, np.linalg.norm(amcl_pose[:, 0:2] - actual_pose[:, 0:2], axis=-1), s=2, marker=marker, label=robot)
